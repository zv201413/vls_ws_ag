...
# --- 3. 环境准备 ---
[ -f "cloudflared" ] || { curl -L -o cloudflared https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64 && chmod +x cloudflared; }
[ -f "xray" ] || { curl -L -o xray.zip https://github.com/XTLS/Xray-core/releases/latest/download/Xray-linux-64.zip && unzip -o xray.zip && chmod +x xray; }
...
if [ "$warp" = "y" ]; then
    ...
    # 核心检查：只有包含 public_key 且不包含 success:false 时才写入
...
echo -e "\n节点链接：\nvless://8e6290c1-b97e-40c0-b9a3-7e7ed11ce248@$ADDRESS:$( [ -n "$DOMAIN" ] && echo "443" || echo "8003" )?encryption=none&security=$( [ -n "$DOMAIN" ] && echo "tls" || echo "none" )&sni=$ADDRESS&type=ws&host=$ADDRESS&path=%2Fws#Argo-VLESS"
